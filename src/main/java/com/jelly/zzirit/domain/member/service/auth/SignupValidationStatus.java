...
enum SignupValidationStatus {
	...
	EMAIL_NOT_VERIFIED {
		@Override
		public void validate(SignupDTO signupDto, MemberRepository memberRepository, RedisService redisService, PasswordManager passwordManager) {
			String emailVerified = redisService.getData("emailAuth:" + signupDto.getMemberEmail() + ":verified");
	...
	PASSWORD_INVALID {
		@Override
		public void validate(SignupDTO signupDto, MemberRepository memberRepository, RedisService redisService, PasswordManager passwordManager) {
			if (passwordManager.isInvalid(signupDto.getMemberPassword())) {
	...
	abstract void validate(SignupDTO signupDto, MemberRepository memberRepository, RedisService redisService, PasswordManager passwordManager);

	public static void validateAll(SignupDTO signupDto, MemberRepository memberRepository, RedisService redisService, PasswordManager passwordManager) {
		for (SignupValidationStatus status : SignupValidationStatus.values()) {
			status.validate(signupDto, memberRepository, redisService, passwordManager);
		}
	}
}